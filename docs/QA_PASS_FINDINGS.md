...
| M-2 | Lockdown-3D   | `NoteReader.cs` | When a paper-note overlay was shown the cursor was unlocked, but `FirstPersonController` was not paused — so look-input from the freed cursor would still rotate the camera behind the overlay. | Initially fixed via `PauseController.Pause()`. Devin Review then flagged that the `PauseMenu` subscribes to `OnPauseChanged` and would surface its own panel on top of the note overlay, and that pressing Resume from that panel left the note overlay visible while the world unfroze. **Final fix**: NoteReader now toggles `FirstPersonController.ControlEnabled` directly, freezing look/move without triggering the global pause flag. Added a `NoteReader.IsOpen` flag for any future code that needs to observe the state. |
...
| M-5 | Lockdown-3D + Containment-VR | `Keypad.cs` line 67, `VRKeypad.cs` line 56 | The wrong-code `FlashRed` coroutine cached the pre-flash light color, then restored it after 0.6 s **without checking Solved**. If the player entered the correct code during the flash window, the accept path set the light to green — but the still-running coroutine then stomped that green back to red 0.6 s later, leaving a misleading "still failed" indicator on a solved puzzle. (Found by Devin Review.) | `FlashRed` now restores the previous color only when `!Solved`. Both keypads patched. |
